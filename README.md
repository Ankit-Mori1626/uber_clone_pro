...
2. **Load Necessary Kernel Modules:** Required for Kubernetes networking.
   ...
   ```bash
   cat <<EOF "$VERSION_CODENAME") "deb $(. && **Install **Set --print-architecture) --system -d -fsSL -m -o -y /etc/apt/keyrings /etc/apt/keyrings/docker.asc /etc/apt/sources.list.d/docker.list /etc/modules-load.d/k8s.conf /etc/os-release /etc/sysctl.d/k8s.conf 0755 3. 4. <<EOF Containerd:** EOF Helps Parameters:** Sysctl [arch="$(dpkg" ``` ```bash a+r apt-get br_netfilter ca-certificates cat chmod curl echo grep [https://download.docker.com/linux/ubuntu](https://download.docker.com/linux/ubuntu) [https://download.docker.com/linux/ubuntu/gpg](https://download.docker.com/linux/ubuntu/gpg) install lsmod modprobe net.bridge.bridge-nf-call-ip6tables="1" net.bridge.bridge-nf-call-iptables="1" net.ipv4.ip_forward="1" networking. overlay signed-by="/etc/apt/keyrings/docker.asc]" stable" sudo sysctl tee update with |> /dev/null

   sudo apt-get update
   sudo apt-get install -y containerd.io

   containerd config default | sed -e 's/SystemdCgroup = false/SystemdCgroup = true/' -e 's/sandbox_image = "registry.k8s.io\/pause:3.9"/sandbox_image = "registry.k8s.io\/pause:3.9"/' | sudo tee /etc/containerd/config.toml
   sudo systemctl restart containerd
